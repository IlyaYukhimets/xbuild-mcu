...
/**
 * Default xmake configuration used as a base for reads and validation.
 * Single source of truth for the "empty" config shape.
 */
export function getDefaultXmakeConfig(): XmakeConfig {
  return {
    PROJECT_NAME: "",
    MCU_SERIES: "",
    MCU_CORE: "",
...
export class XmakeConfigParser {
  ...
  constructor(workspacePath: string) {
    this.xmakePath = join(workspacePath, "xmake.lua");
  }

  public exists(): boolean {
    return existsSync(this.xmakePath);
  }

  public read(): XmakeConfig {
    if (!existsSync(this.xmakePath)) {
      return getDefaultXmakeConfig();
    }

    const content = readFileSync(this.xmakePath, "utf-8");
    ...
    return {
      ...
      OPTIMIZATION_DEBUG:
        this.extractVariable(content, "OPTIMIZATION_DEBUG") || "debug",
      OPTIMIZATION_RELEASE:
        this.extractVariable(content, "OPTIMIZATION_RELEASE") || "release",
    };
  }
   ...
   */
  private extractRawValuesFromCalls(
    content: string,
    calls: Array<{ argsStart: number; argsEnd: number }>,
  ): LuaValue[] {
  ...
  private ensureVariableExists(
    ...
    if (!regex.test(content)) {
      ...
      const lastLocalMatch = content.match(
        /local\s+\w+\s*=\s*["'][^"']*["']/gi,
      );
      if (lastLocalMatch) {
  ...
  public write(config: XmakeConfig): boolean {
    try {
      let content = readFileSync(this.xmakePath, "utf-8");

      // Ensure optimization variables exist
      content = this.ensureVariableExists(
        content,
        "OPTIMIZATION_DEBUG",
      ...
      content = this.ensureVariableExists(
        content,
        "OPTIMIZATION_RELEASE",
        config.OPTIMIZATION_RELEASE,
      );

      // Update variables
      content = this.updateVariable(
        content,
        "PROJECT_NAME",
        config.PROJECT_NAME,
      );
      ...
      content = this.updateArrayWithRaw(
        content,
        "add_includedirs",
        config.INCLUDE_DIRS,
      );
      ...
      writeFileSync(this.xmakePath, content, "utf-8");
      return true;
    } catch (error) {
      logger.error("Failed to write xmake.lua", error);
      return false;
    }
  }
  ...
  private updateArrayWithRaw(
    ...
    for (let i = 0; i < sortedCalls.length; i++) {
      ...
      if (i === 0) {
        ...
        result =
          result.slice(0, adjustedStart) + newCall + result.slice(adjustedEnd);
        ...
        if (before.includes("\n")) {
          ...
          result =
            result.slice(0, beforeStart + newlinePos) +
