...
/**
 * Extract stderr from a promisified-exec rejection, if present.
 */
...
function getSubmoduleRepos(): GitRepository[] {
  const userRepos = getWorkspaceConfig<GitRepository[]>("submoduleRepos", []);

  // Validate and filter repos
  return userRepos.filter((repo) => {
    if (!repo.name || !repo.url) {
      logger.warning(
        "Invalid submodule repo in config (missing name or url)",
        repo,
      );
      return false;
    }
...
export class XmakeConfigPanel implements vscode.Disposable {
  ...
  private async handleMessage(message: {
    ...
    switch (message.command) {
      ...
      case "getSubmodules": {
        ...
        const installed = isGit
          ? await this.getInstalledSubmodules()
          : new Map();
        this.panel.webview.postMessage({
  ...
  private async initGitRepository(): Promise<boolean> {
    try {
      ...
      const errorMsg = execStderr(error) || getErrorMessage(error);
  ...
  private async getInstalledSubmodules(): Promise<Map<string, string>> {
    ...
    try {
      const stdout = await execSilent("git submodule status", {
  ...
  private async addSubmodule(
    ...
    try {
      ...
      if (stderr.includes("already exists")) {
        ...
        stderr.includes("git directory for") &&
        stderr.includes("is found locally")
      ) {
        ...
        if (force === "Force Add") {
          try {
            await execAsync(
              `git submodule add --force "${url}" "${submodulePath}"`,
              { cwd: this.workspacePath },
            );
            vscode.window.showInformationMessage(
              "Submodule added (forced): " + submodulePath,
            );
            logger.info("Submodule added (forced)", {
              path: submodulePath,
              url,
            });
            return true;
            ...
            vscode.window.showErrorMessage(
              "Failed to force add submodule: " +
                (execStderr(forceError) || getErrorMessage(forceError)),
            );
            return false;
          }
        }
  ...
  private async removeSubmodule(submodulePath: string): Promise<boolean> {
    ...
    if (existsSync(modulesPath)) {
      ...
      if (!removed) {
        vscode.window.showWarningMessage(
          "Could not fully clean .git/modules directory. You may need to manually remove: " +
            modulesPath,
        );
      }
    }
    ...
    const gitmodulesPath = join(this.workspacePath, ".gitmodules");
    if (existsSync(gitmodulesPath)) {
      try {
        let content = readFileSync(gitmodulesPath, "utf-8");
        ...
        if (content.trim().length === 0) {
          ...
          try {
            await execAsync("git rm -f .gitmodules", {
              cwd: this.workspacePath,
            });
          } catch {
            /* ignore */
          ...
          writeFileSync(gitmodulesPath, content, "utf-8");
        }
      } catch (error) {
  ...
  public static createOrShow(
    ...
    XmakeConfigPanel.currentPanel = new XmakeConfigPanel(
      panel,
      extensionUri,
      workspacePath,
    );
